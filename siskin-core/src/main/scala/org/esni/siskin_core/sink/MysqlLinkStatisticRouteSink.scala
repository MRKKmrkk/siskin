...
class MysqlLinkStatisticRouteSink extends RichSinkFunction[LinkStatisticRouteView] {
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    ps = connection.prepareStatement("insert into link_statistic(serverAddress, request, statistic_count, statistic_time) values(?, ?, ?, ?)")
