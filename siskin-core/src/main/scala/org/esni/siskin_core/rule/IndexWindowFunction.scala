...
// [IN, OUT, KEY, W <: Window]
class IndexWindowFunction(ruleConfig: JSONObject, sirId: Int) extends WindowFunction[AggregateIndexes, Spider, String, TimeWindow] with Logging{
      ...
      .foreach {
        aggregateIndexes =>
          if (ruleGroupController.getFlag(aggregateIndexes.userAgentCount, aggregateIndexes.cookieCount, aggregateIndexes.visitTimesCount, aggregateIndexes.miniNumAccessIntervalTimesCount)){
            out.collect(Spider(key, sirId, 0L))
          }
      }
